...
jest.mock('../src/native/NearbyConnections', () => {
  const connectionRequested: Array<(request: {endpointId: string; name: string}) => void> = [];
  return {
    NearbyConnections: {
      start: jest.fn(),
      stop: jest.fn(),
      ...
      onConnectionRequested: (listener: (request: {endpointId: string; name: string}) => void) => {
...
type MockedNearbyConnections = typeof NearbyConnections & {
  __mockListeners: {
    connectionRequested: Array<(request: {endpointId: string; name: string}) => void>;
  };
};

const mockListeners = (NearbyConnections as MockedNearbyConnections).__mockListeners;
const mockRespondToConnection = NearbyConnections.respondToConnection as jest.MockedFunction<
...
test('shows an incoming Nearby connection request and lets the user accept it', async () => {
  ...
  await act(async () => {
    mockListeners.connectionRequested[0]({endpointId: 'peer-1', name: 'Protidhoni-abcxyz'});
  });
  ...
  expect(renderer!.root.findAllByProps({testID: 'accept-peer-1'})).toHaveLength(0);
});
...
test('lets the user decline an incoming Nearby connection request', async () => {
  ...
  await act(async () => {
    mockListeners.connectionRequested[0]({endpointId: 'peer-2', name: 'Protidhoni-zzz'});
  });
  ...
  expect(renderer!.root.findAllByProps({testID: 'decline-peer-2'})).toHaveLength(0);
});
