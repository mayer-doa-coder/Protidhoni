...
/docs/              Architecture notes, acceptance records.
Protidhoni_Roadmap.md   The full build plan this repository implements, phase by phase.
```

## 4. Implementation status

Every phase below is **built, tested, and independently verified** — not just claimed. "Verified" means: the real test suite was actually executed and passed, or (for the cross-service claims) two independent real implementations were run together in the same process and their output was compared byte-for-byte, not asserted from documentation.

| Phase | Scope | Status |
|---|---|---|
| **0** | Frozen contract, scaffolded backend/mobile/AI/dashboard, Nearby discovery proven on two physical devices | ✅ Done |
| **1** | `POST/GET /reports`, mobile SOS creation + queue + mesh relay + sync, basic classifier, map dashboard | ✅ Done |
...
| **4** | SMS + offline-USSD-simulator gateway, gateway-signed reports, independent adapter auth, phone-number minimisation | ✅ Done |
| **5** | Frozen LoRa transport frame, simulated Meshtastic sender/gateway/reassembly, Meshtasticator scenario matrix + Site Planner propagation study | ✅ Done (software/protocol simulation only — see §8) |
| **6** | Integration/system testing across every service boundary | ✅ This section — see §6 |
...
| `ai-service/tests` | 23 | Classification, translation boundary, internal-token auth |
| `dashboard` (vitest) | 26 | Filtering, clustering, channel provenance, API typing |
| `mobile-client` (jest) | 116 | Signing, canonicalization, SQLite queue, mesh relay, forms, sync, screens |
...
| **`tests/system`** (new) | **27** | **Cross-service: real signed report through real `POST /reports`; the full sign→LoRa-encode→reassemble→real-backend chain in one process; real SMS/USSD HTTP requests against the real gateway; backend's translation client against ai-service's real FastAPI app; mobile-client's actual `@noble/ed25519`/`canonicalize` libraries cross-verified against the backend's actual `cryptography`/`rfc8785` libraries** |
| **Total** | **485** | |
...
```powershell
# Backend
cd backend; .\.venv\Scripts\Activate.ps1; python -m pytest tests -q
...
# Mobile client
cd mobile-client; npm test; npx tsc --noEmit

# Phase 5 (each package has its own venv — see hardware/README.md)
cd hardware/protocol; .\.venv\Scripts\python.exe -m pytest
cd hardware/gateway; .\.venv\Scripts\python.exe -m pytest
cd hardware/simulation; python -m pytest tests -q

# Cross-service integration/system suite — see tests/system/README.md for setup
cd tests/system; .\.venv\Scripts\python.exe -m pytest -q
```
...
- **A real cross-platform build reproducibility bug**, found and fixed during this pass: `git config core.autocrlf=true` (the common Windows default) was silently rewriting every Phase 5 evidence file's line endings on checkout, invalidating the recorded SHA-256 integrity hashes the project's own evidence validator checks. Fixed with `.gitattributes` pinning those paths to `-text`, verified by re-running the validator clean.

## 7. Running the full stack

```powershell
copy .env.example .env
# edit .env: set every placeholder secret to a real local-only value
...
- **Android is the only mobile target.** Nearby Connections supports iOS, but the Phase 0 bridge and every device test cover Android only.
